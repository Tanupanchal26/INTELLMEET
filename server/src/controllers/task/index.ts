...
const isAdminOrOwner = (task: any, req: Request) => {
  const isAdmin = ['admin', 'super_admin'].includes(String(req.user?.role ?? ''));
  const isOwner = String(task.createdBy) === String(req.user?._id ?? '');
  return isAdmin || isOwner;
};

const logActivity = async (taskId: string, teamId: string | null, tenantId: string, actor: string, action: string, meta: object = {}) => {
  try {
...
exports.getTeamTasks = asyncHandler(async (req: Request, res: Response) => {
  const { teamId } = req.params;

  // Verify membership
  const team = await Team.findOne({ _id: teamId, tenantId: req.tenantId });
  if (!team) throw ApiError.notFound('Team not found');
  const isMember = team.members.some((m: any) => String(m.user) === String(req.user?._id));
  if (!isMember) throw ApiError.forbidden('Not a team member');

  const filter: Record<string, unknown> = { tenantId: req.tenantId, teamId };
  if (req.query.status)   filter.status   = req.query.status;
  if (req.query.priority) filter.priority = req.query.priority;
...
exports.updateTask = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!isAdminOrOwner(task, req)) throw ApiError.forbidden('Not authorised to modify this task');
  ...
  const historyEntries = TRACKED
    .filter(f => safeBody[f] !== undefined && String(safeBody[f]) !== String((task as any)[f]))
    .map(f => ({ changedBy: req.user?._id, field: f, from: (task as any)[f], to: safeBody[f], at: new Date() }));

  const updated = await Task.findByIdAndUpdate(
    task._id,
    { ...safeBody, $push: historyEntries.length ? { history: { $each: historyEntries } } : {} },
...
/* ── Task history ───────────────────────────────────────────────────────────── */
exports.getTaskHistory = asyncHandler(async (req: Request, res: Response) => {
  const task = await Task.findOne({ _id: req.params.id, tenantId: req.tenantId })
    .select('history')
...
exports.getTeamActivity = asyncHandler(async (req: Request, res: Response) => {
  const { teamId } = req.params;
  const team = await Team.findOne({ _id: teamId, tenantId: req.tenantId });
  if (!team) throw ApiError.notFound('Team not found');
  const isMember = team.members.some((m: any) => String(m.user) === String(req.user?._id));
