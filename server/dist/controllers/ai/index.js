...
exports.getFullReport = asyncHandler(async (req, res) => {
    const result = await AIResult.findOne({ meeting: req.params.meetingId })
        ...
        .lean();
    if (!result)
        throw ApiError.notFound('AI result not found');
...
exports.getAIResult = asyncHandler(async (req, res) => {
    const result = await AIResult.findOne({ meeting: req.params.meetingId });
    if (!result)
        return ApiResponse.ok(res, { meeting: req.params.meetingId, processingStatus: 'idle', summary: '', transcript: '', actionItems: [], minutes: '' }, 'No AI result yet');
...
exports.generateSummary = asyncHandler(async (req, res) => {
    ...
    if (transcript) {
        await AIResult.findOneAndUpdate({ meeting: req.params.meetingId }, { meeting: req.params.meetingId, transcript }, { upsert: true, new: true });
    }
    const summary = await aiService.summarize(req.params.meetingId, length);
    if (!summary) {
        return res.status(400).json(new ApiResponse(400, "Meeting transcript is empty. Generate or upload a transcript before requesting a summary."));
    }
...
exports.deleteSummary = asyncHandler(async (req, res) => {
    await AIResult.findOneAndUpdate({ meeting: req.params.meetingId }, { summary: '', summaryLength: 'medium', $inc: { version: 1 } });
...
exports.getActionItems = asyncHandler(async (req, res) => {
    ...
    if (!result?.transcript && !result?.transcriptChunks?.length)
        return res.status(400).json(new ApiResponse(400, "Meeting transcript is empty. Generate or upload a transcript before requesting action items."));
...
exports.updateActionItem = asyncHandler(async (req, res) => {
    ...
    if (text !== undefined)
        item.text = String(text).slice(0, 300);
...
exports.generateMinutes = asyncHandler(async (req, res) => {
    ...
    if (!hasTranscript)
        throw ApiError.badRequest('No transcript found. Generate or upload a transcript first.');
    ...
    if (job) {
        return res.status(202).json(new ApiResponse(202, `Minutes generation queued (job: ${job.id})`, { jobId: job.id }));
    }
...
exports.assistantChat = asyncHandler(async (req, res) => {
    ...
    try {
        const reply = await aiService.assistantChat(req.params.meetingId, req.user?.tenantId ?? '', message, context?.history || []);
...
exports.generateTasks = asyncHandler(async (req, res) => {
    ...
    if (!prompt && !transcript)
        throw ApiError.badRequest('prompt or transcript is required');
    if (prompt && prompt.length > MAX_PROMPT)
        throw ApiError.badRequest(`prompt exceeds ${MAX_PROMPT} character limit`);
    if (transcript && transcript.length > MAX_TRANSCRIPT)
        throw ApiError.badRequest(`transcript exceeds ${MAX_TRANSCRIPT} character limit`);
...
exports.extractAndSaveTasks = asyncHandler(async (req, res) => {
    ...
    const tasks = await Task.insertMany(actionItems.map((item) => ({
...
exports.searchMeetings = asyncHandler(async (req, res) => {
    const { q } = req.query;
    if (!q)
        throw ApiError.badRequest('query param q is required');
    if (String(q).length > 500)
        throw ApiError.badRequest('query too long');
...
exports.getMeetingHistory = asyncHandler(async (req, res) => {
    ...
    const data = await aiService.getMeetingHistory(req.user?.tenantId, Number(page), Math.min(Number(limit), 100));
