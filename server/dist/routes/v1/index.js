...
const { authenticate, authorize, protect, scopeTenant } = auth_middleware_1.default;
...
const googleAuthCtrl = require('../../controllers/googleAuth');
...
const transcriptionCtrl = require('../../controllers/transcription');
const taskCtrl = require('../../controllers/task');
const analyticsCtrl = require('../../controllers/analytics');
const exportCtrl = require('../../controllers/export');
const recordingCtrl = require('../../controllers/recording');
const mediaCtrl = require('../../controllers/media');
// Validators
const AV = __importStar(require("../../validators/auth.validator"));
const UV = __importStar(require("../../validators/user.validator"));
const MV = __importStar(require("../../validators/meeting.validator"));
const TV = __importStar(require("../../validators/team.validator"));
const CV = __importStar(require("../../validators/channel.validator"));
const NV = __importStar(require("../../validators/notification.validator"));
const constants_1 = require("../../constants");
const router = (0, express_1.Router)();
const upload = (0, multer_1.default)({ storage: multer_1.default.memoryStorage() });
// ── Google OAuth ──────────────────────────────────────────────────────────────
router.get('/auth/google', passport.authenticate('google', { scope: ['profile', 'email'], prompt: 'select_account' }));
router.get('/auth/google/callback', passport.authenticate('google', { session: false, failureRedirect: `${process.env.CLIENT_URL}/login?error=google_failed` }), googleAuthCtrl.googleCallback);
// ── Auth ──────────────────────────────────────────────────────────────────────
...
router.delete('/teams/:id/members/:userId', authorize(constants_1.ROLES.ADMIN), validate(TV.memberParam), teamCtrl.removeMember);
...
router.put('/channels/:id', authorize(constants_1.ROLES.ADMIN), validate(CV.updateChannel), channelCtrl.updateChannel);
router.delete('/channels/:id', authorize(constants_1.ROLES.ADMIN), validate(CV.channelParam), channelCtrl.archiveChannel);
...
// ── Team Chat ─────────────────────────────────────────────────────────────────
const teamChatCtrl = require('../../controllers/teamChat');
...
router.get('/tenants', authorize('super_admin'), async (req, res, next) => {
    try {
        res.json(await Tenant.find().select('-__v'));
    }
    catch (e) {
        next(e);
    }
...
router.get('/tenants/me', authenticate, async (req, res, next) => {
    try {
        const t = await Tenant.findById(req.tenantId);
        if (!t)
...
router.patch('/tenants/me/settings', authorize(constants_1.ROLES.ADMIN, constants_1.ROLES.SUPER_ADMIN), async (req, res, next) => {
    try {
        res.json(await Tenant.findByIdAndUpdate(req.tenantId, { $set: { settings: req.body } }, { new: true, runValidators: true }));
    }
    catch (e) {
        next(e);
    }
