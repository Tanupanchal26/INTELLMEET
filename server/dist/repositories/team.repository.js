...
class TeamRepository extends BaseRepository {
    ...
    async addMember(teamId, tenantId, userId, role = 'member', status = 'active') {
        ...
        const team = await Team.findOneAndUpdate(filter, { $push: { members: { user: userId, role, status } } }, { new: true });
        if (!team)
            throw ApiError.conflict('User is already a member or team not found');
        return team;
    }
    async updateMemberStatus(teamId, tenantId, userId, status) {
        const team = await Team.findOneAndUpdate({ _id: teamId, tenantId, 'members.user': userId }, { $set: { 'members.$.status': status } }, { new: true });
        if (!team)
            throw ApiError.notFound('Team member not found');
        return team;
    }
    async removeMember(teamId, tenantId, userId) {
        const team = await Team.findOneAndUpdate({ _id: teamId, tenantId }, { $pull: { members: { user: userId } } }, { new: true });
        if (!team)
            throw ApiError.notFound('Team not found');
        return team;
    }
    async updateMemberRole(teamId, tenantId, userId, role) {
        const team = await Team.findOneAndUpdate({ _id: teamId, tenantId, 'members.user': userId }, { $set: { 'members.$.role': role } }, { new: true });
        if (!team)
            throw ApiError.notFound('Team member not found');
        return team;
    }
