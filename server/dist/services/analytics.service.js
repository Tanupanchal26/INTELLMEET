...
exports.getDashboardMetrics = async (tenantId, userId) => {
    ...
    let tid;
    try {
        tid = new mongoose.Types.ObjectId(tenantId);
    }
    catch {
        tid = null;
    }
    ...
    const [taskData, recentMeetings, upcomingMeetings, meetingStats, meetingHoursAgg, aiSummariesAgg, rawActivity,] = await Promise.all([
        Task.aggregate([
