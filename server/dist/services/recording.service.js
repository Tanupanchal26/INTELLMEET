...
exports.uploadRecording = async (meetingId, tenantId, ownerId, buffer, sizeBytes) => {
    ...
    return new Promise((resolve, reject) => {
        const stream = cloudinary.uploader.upload_stream({ resource_type: 'video', folder: `intellmeet/recordings/${tenantId}` }, async (error, result) => {
            if (error)
                return reject(ApiError.internal(`Cloudinary upload failed: [${error.http_code}] ${error.message}`));
            try {
