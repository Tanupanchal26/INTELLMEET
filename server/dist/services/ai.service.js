...
exports.saveTranscript = async (meetingId, transcript) => {
    await AIResult.findOneAndUpdate({ meeting: meetingId }, { meeting: meetingId, transcript, processingStatus: 'idle' }, { upsert: true, new: true });
    await cacheDel(`ai:summary:${meetingId}`, `ai:minutes:${meetingId}`, `ai:keywords:${meetingId}`, `ai:smartnotes:${meetingId}`);
};
// ── Summarize ─────────────────────────────────────────────────────────────────
exports.summarize = async (meetingId, length = 'medium') => {
    ...
    try {
        ...
        await AIResult.findOneAndUpdate({ meeting: meetingId }, { summary, summaryLength: length, actionItems, decisions, processingStatus: 'completed', $inc: { version: 1 } }, { upsert: true });
        ...
        await AIResult.findOneAndUpdate({ meeting: meetingId }, { processingStatus: 'failed', processingError: err.message });
        throw err;
    }
};
// ── Action Items ──────────────────────────────────────────────────────────────
...
// ── Decisions ─────────────────────────────────────────────────────────────────
exports.getDecisions = async (meetingId) => {
    const result = await AIResult.findOne({ meeting: meetingId });
...
// ── Keywords ──────────────────────────────────────────────────────────────────
exports.getKeywords = async (meetingId) => {
...
// ── Smart Notes ───────────────────────────────────────────────────────────────
exports.getSmartNotes = async (meetingId) => {
    ...
    if (!transcript)
        return null;
    const agendaItems = (meeting?.agenda || []).map((a) => a.title).filter(Boolean);
...
exports.getFollowUpSuggestions = async (meetingId) => {
...
exports.runFullPipeline = async (meetingId) => {
    const [meeting, transcript] = await Promise.all([
    ...
    await AIResult.findOneAndUpdate({ meeting: meetingId }, { processingStatus: 'processing', participants: (meeting.participants || []).map((p) => p._id) }, { upsert: true });
    try {
        const [summary, actionItems, decisions, keywords, followUpSuggestions, minutes, smartNotes] = await Promise.all([
            ...
            generateSmartNotes({
                transcript,
                title: meeting.title,
                agenda: (meeting.agenda || []).map((a) => a.title).filter(Boolean),
            }),
        ]);
        ...
        await Meeting.findByIdAndUpdate(meetingId, { summary });
        // Bust all caches
        await cacheDel(`ai:summary:${meetingId}:medium`, `ai:keywords:${meetingId}`, `ai:smartnotes:${meetingId}`, `ai:minutes:${meetingId}`, `ai:followup:${meetingId}`);
        // Notify participants: summary ready
        const participantIds = [
            ...new Set([
                String(meeting.host),
                ...(meeting.participants || []).map((p) => String(p._id || p)),
            ]),
        ];
...
// ── History ───────────────────────────────────────────────────────────────────
exports.getMeetingHistory = async (tenantId, page = 1, limit = 20) => {
