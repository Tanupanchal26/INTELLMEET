...
const createTeam = async (tenantId, userId, data) => {
    ...
    if (existing)
        throw ApiError.conflict(`Team slug "${slug}" already exists`);
...
const getTeam = async (teamId, tenantId, userId) => {
    const team = await teamRepo.findById(teamId, tenantId, [{ path: 'members.user', select: 'name email avatar' }]);
    // Private teams: only members can view
    if (team.isPrivate) {
        const role = teamRepo.getMemberRole(team, userId);
        if (!role)
            throw ApiError.forbidden('This team is private');
    }
    return team;
};
// ── Update team ───────────────────────────────────────────────────────────────
...
const inviteMemberByEmail = async (teamId, tenantId, actorId, email, role = 'member') => {
    ...
    if (!targetUser)
        throw ApiError.notFound('No registered user found with this email');
...
const inviteMember = async (teamId, tenantId, actorId, targetUserId, role = 'member') => {
    ...
    if (general)
        await channelRepo.addMember(general._id, targetUserId);
    // Notify
...
const rejectInvitation = async (teamId, tenantId, userId) => {
    ...
    if (!member)
        throw ApiError.notFound('No invitation found for this team');
    if (member.status === 'active')
        throw ApiError.badRequest('Cannot reject — already an active member');
...
const acceptInvitation = async (teamId, tenantId, userId) => {
    ...
    if (!member)
        throw ApiError.notFound('No invitation found for this team');
    if (member.status === 'active')
        throw ApiError.badRequest('You are already an active member of this team');
    const updated = await teamRepo.updateMemberStatus(teamId, tenantId, userId, 'active');
    const general = await channelRepo.findBySlug(team._id, 'general');
    if (general)
        await channelRepo.addMember(general._id, userId);
    return updated;
};
// ── Remove member ─────────────────────────────────────────────────────────────
...
const updateMemberRole = async (teamId, tenantId, actorId, targetUserId, role) => {
    ...
    if (role === 'owner')
        throw ApiError.badRequest('Cannot assign owner role via API');
