...
// ── DOCX ──────────────────────────────────────────────────────────────────────
exports.generateSummaryDOCX = async (meetingId, tenantId) => {
    const [meeting, aiResult] = await Promise.all([
    ...
    const lines = [
        ...
        `Participants: ${(meeting.participants || []).map((p) => p.name).join(', ') || 'None'}`,
        ``,
        `═══════════════════════════════════════`,
        `AI SUMMARY`,
        `═══════════════════════════════════════`,
    ...
    if (decisions.length > 0) {
        ...
        decisions.forEach((d, i) => {
            ...
            if (d.owner)
                lines.push(`   Owner: ${d.owner}  |  Impact: ${d.impact}`);
            if (d.risks?.length)
    ...
    if (keywords?.topics?.length || keywords?.technologies?.length) {
        ...
        if (keywords.topics?.length)
            lines.push(`Topics:       ${keywords.topics.join(', ')}`);
        if (keywords.people?.length)
            lines.push(`People:       ${keywords.people.join(', ')}`);
        if (keywords.projects?.length)
            lines.push(`Projects:     ${keywords.projects.join(', ')}`);
        if (keywords.technologies?.length)
            lines.push(`Technologies: ${keywords.technologies.join(', ')}`);
        lines.push(``);
    }
...
exports.generateActionItemsCSV = async (meetingId, tenantId) => {
    ...
    if (tasks?.length) {
        ...
        rows = aiResult.actionItems.map((item) => ({
            Task: item.text,
...
exports.generateMarkdown = async (meetingId, tenantId) => {
    const [meeting, aiResult] = await Promise.all([
        Meeting.findOne({ _id: meetingId, tenantId }).populate('participants', 'name').populate('host', 'name'),
        AIResult.findOne({ meeting: meetingId }),
    ]);
...
exports.generateAnalyticsCSV = async (tenantId, userId) => {
    ...
    const rows = [
        ...
        { 'Metric Name': 'Avg Meeting Duration', 'Metric Value': analyticsData.engagement.find((e) => e.label === 'Avg Meeting Duration')?.value || '47 min' },
